...
var ExecutionBranch = require('./Transition/Branch').ExecutionBranch
...
function Transition (options) {
  ...
  function run () {
    ...
    var execution = new ExecutionBranch(options)
    execution
      .run(origin, options.hints, token)
      .then(factory(Status.Executed), factory(Status.ExecutionFailure))
      .then(function (result) {
        var level = result.successful ? 'debug' : 'warn'
        var term = result.successful ? 'success' : 'error'
