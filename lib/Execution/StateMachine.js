...
function StateMachine (executor, scenario, loggerOpts) {
  ...
  function processSuccess (t8n, value) {
    ...
    if (state.terminal) {
      logger.info('State {} is terminal, halting any further processing',
        state.id)
      terminate(Stage.Finished, OperationStatus.Finished, value)
      return
    }
  ...
  function processError (t8n, error) {
    ...
    return future.then(terminate.bind(null, stage, status, error))
  }
   ...
   *
   * @param {StateMachine.Stage} stage
   * @param {OperationStatus} status
   * @param {*} [value]
   */
  function terminate (stage, status, value) {
    setStatus(Stage.Terminating)
  ...
  this.terminate = function () {
    ...
    abort()
    terminate(Stage.Finished, OperationStatus.Aborted, null)
    return termination
  }
 ...
 */
var statusFactory = function (name, closed, terminal) {
  var active = !closed
  terminal = typeof terminal === 'boolean' ? terminal : !active
  return {
    name: name,
    active: active,
    terminal: terminal
...
/**
 * TODO: revise stages since main meaning is now held in OperationStatus
 *
...
StateMachine.Stage = {
  ...
  Terminating: statusFactory('Terminating', true, false),
  ErrorHandling: statusFactory('ErrorHandling', true, false),
