...
/**
 * @param {object.<StateId, State>} states
 * @param {IExecutor} executor
 * @param {StateMachine~errorHandler} [errorHandler]
...
function StateMachine (executor, states, errorHandler, loggerOpts) {
  ...
  function processSuccess (t8n, value) {
    ...
    if (state.terminal) {
      logger.info('State {} is terminal, halting any further processing',
        state.id)
      terminate(Status.Finished, value)
      return
    }
   ...
   *
   * @param {StateMachine.Status} status
   * @param {*} [value]
   */
