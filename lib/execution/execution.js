...
var ExecutionResult = schema.ExecutionResult
 ...
 * @class
 *
 * @param {TScenario} scenario Scenario to execute.
...
function Execution (scenario, machine, runtime, logger) {
  ...
  var termination = completion.then(terminate, function (e) { return terminate(ExecutionResult.failure(e)) })
  var status = ExecutionStatus.Idle
  var scenarioTimeout
  ...
  function runTerminationHandler () {
    ...
    return timeout(future, scenario.timeouts.onTermination, function (res, rej, e) {
      ...
      var rescue = runtime.execute(scenario.onTerminationTimeout, localToken, e)
