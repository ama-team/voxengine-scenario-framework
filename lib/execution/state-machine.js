...
function StateMachine (states, runtime, logger) {
  ...
  var scenarioState
  var scenarioStateTimeout
  var active
  var history = []
  var completion = new CompletablePromise()
  var termination = completion.then(terminate, terminate)
  var transitions = []
  var entrypoint = states.reduce(function (c, s) { return s.entrypoint ? s : c }, null)
  ...
  function requireState (id) {
    var s = getState(id)
    if (!s) { throw new Error('TState ' + id + ' is not registered') }
    return s
  }
