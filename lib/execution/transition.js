...
var timeout = concurrent.safeTimeout
var CompletablePromise = concurrent.CompletablePromise
...
var TransitionResult = schema.TransitionResult
...
function Transition (origin, target, hints, runtime, logger) {
  ...
  function runRescueHandler (error) {
    ...
    var future = runtime.execute(target.onTransitionTimeout, origin, hints, token, error)
  ...
  function run () {
    ...
    try {
      ...
      var token = new CancellationToken(cancellation)
      var future = runtime.executeAsPromise(target.transition, origin, hints, token)
      ...
      future.then(function (v) {
        ...
        var cause = timedOut ? TerminationCause.TransitionTimeout : TerminationCause.TransitionFailure
  ...
  function abort () {
    ...
    try {
      ...
      var future = runtime.execute(target.abort, origin, hints, token)
      ...
      future
        .then(function (v) {
          ...
          var timedOut = e instanceof TimeoutException
          var cause = timedOut ? TerminationCause.AbortTimeout : TerminationCause.AbortFailure
          state = timedOut ? TransitionState.TimedOut : TransitionState.Failed
          completion.resolve(new TransitionResult(cause, null, e))
        })
    } catch (e) {
  ...
  function runAbortRescueHandler (error) {
    var token = new CancellationToken()
    var future = runtime.execute(target.onAbortTimeout, origin, hints, token, error)
