...
function run(scenario, settings) {
    ...
    execute(scenario, container, deserializeArguments(VoxEngine.customData(), settings), data, logLevel);
}
...
function execute(execution) {
    return execution.run().then(function (result) {
        if (!result.success) {
            var tError = result.terminationError;
            Logger.write('Scenario has terminated unsuccessfully with cause `' + result.cause + '`');
            Logger.write('Scenario error: ' + result.error ? JSON.stringify(result.error) : '(none)');
            Logger.write('Scenario termination error: ' + tError ? JSON.stringify(tError) : '(none)');
        }
        VoxEngine.terminate();
        return result;
    }, function (error) {
        Logger.write('Scenario has terminated with an error: ' + JSON.stringify(error));
        VoxEngine.terminate();
        return error;
    });
}
