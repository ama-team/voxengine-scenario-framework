...
var ExecutionRuntime = require('./execution/runtime').ExecutionRuntime
var StateMachine = require('./execution/state-machine').StateMachine
var schema = require('./schema/definitions')
var TriggerType = schema.TriggerType
var validator = require('./schema/validator')
var normalizer = require('./schema/normalizer').Schema
 ...
 *
 * @param {TScenario|object} scenario Scenario (usually, non-normalized) that will be used by execution.
 * @param {object} [container] Dependency injection container.
 * @param {object} [logLevel] Log level to use.
 * @param {object} [args] Arguments to run scenario with, this is usually fetched from custom data.
 * @param {object} [data] Pre-populated runtime data object.
 *
 * @return {Execution}
 */
function compose (scenario, container, args, data, logLevel) {
  var validation
  var runtime
  var logger
  var machine
  ...
  container = container || {}
  if (!container.logger) {
    container.logger = new slf4j.Slf4j('ama-team.vsf', logLevel)
  }
  logger = container.logger

  violations = Object
    .keys(validation.violations)
    .map(function (k) {
  ...
  runtime = new ExecutionRuntime({logger: logger}, args, data)
 ...
 *
 * @param {TScenario|object} scenario Scenario (normalized or not) that has to be executed.
...
function run (scenario, settings) {
  ...
  var args = deserializeArguments(VoxEngine.customData(), settings, container.logger)
  var execution = compose(scenario, container, args, settings.data || {}, settings.logLevel)
