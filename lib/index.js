var Execution = require('./execution/execution').Execution,
    ...
    loggers = require('@ama-team/voxengine-sdk').logger,
...
function compose(scenario, container, args, data, logLevel) {
    ...
    logLevel = typeof logLevel === 'undefined' ? LogLevel.Info : logLevel;
    container = container || {};
    if (!container.logger) {
        container.logger = new Slf4j('ama-team.voxengine-scenario-framework', logLevel);
    }
...
function run(scenario, settings) {
    ...
    VoxEngine.addEventListener(AppEvents.Started, function (e) { execution.getRuntime().start = e; });
    if (scenario.trigger === TriggerType.Call) {
        /** @param {AppEvents.CallAlerting} event */
        var handler = function (event) {
            execution.getRuntime().arguments = deserializeArguments(event.call.customData(), settings);
            execution.getRuntime().trigger = event;
            execute(execution);
        };
