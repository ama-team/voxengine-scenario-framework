...
var Validator = {
  ...
  scenario: function (input) {
    ...
    if (!input.deserializer) {
      violation = 'Argument deserializer is not set, default will be used'
      violations.push('$.deserializer', violation, Severity.Minor)
    } else if (!Objects.isFunction(input.deserializer)) {
      violation = 'Argument deserializer is not a function'
      violations.push('$.deserializer', violation, Severity.Fatal)
    }
